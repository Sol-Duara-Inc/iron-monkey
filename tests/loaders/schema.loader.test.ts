...
import { loadSchemasFromDir, getDefaultSchemasDir } from '../../src/loaders/schema.loader.js';
...
describe('loadSchemasFromDir', () => {
  ...
  it('skips schemas without a type string', async () => {
    const dir = await makeTmpDir();
  ...
  it('extracts the type string and loads the schema', async () => {
    const dir = await makeTmpDir();
    const schema = {
      properties: {
        context: {
...
describe('getDefaultSchemasDir', () => {
  it('returns the correct default directory', async () => {
