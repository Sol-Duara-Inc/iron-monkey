...
```json
"type": {
  "enum": ["dev.cdevents.pipelinerun.finished.0.5.1"]
}
```

any workflow event with `type: dev.cdevents.pipelinerun.finished.0.5.1` will be validated against that schema, regardless of what the file is named.

## Bundled schemas

Iron Monkey ships with 45 CDEvents 0.5.1 schemas covering all standard event types. A representative sample:

```
pipelinerunstarted.json    →  dev.cdevents.pipelinerun.started.0.5.1
pipelinerunqueued.json     →  dev.cdevents.pipelinerun.queued.0.5.1
pipelinerunfinished.json   →  dev.cdevents.pipelinerun.finished.0.5.1
buildstarted.json          →  dev.cdevents.build.started.0.5.1
buildfinished.json         →  dev.cdevents.build.finished.0.5.1
testsuiterunstarted.json   →  dev.cdevents.testsuiterun.started.0.5.1
testsuiterunfinished.json  →  dev.cdevents.testsuiterun.finished.0.5.1
artifactpackaged.json      →  dev.cdevents.artifact.packaged.0.5.1
artifactpublished.json     →  dev.cdevents.artifact.published.0.5.1
taskrunstarted.json        →  dev.cdevents.taskrun.started.0.5.1
taskrunfinished.json       →  dev.cdevents.taskrun.finished.0.5.1
servicedeployed.json       →  dev.cdevents.service.deployed.0.5.1
servicepublished.json      →  dev.cdevents.service.published.0.5.1
...
```
...
Schemas must follow the CDEvents 0.5.1 shape and use **JSON Schema 2020-12** (`"$schema": "https://json-schema.org/draft/2020-12/schema"`):
...
```
Error: No schema found for event type 'dev.cdevents.foo.bar.0.5.1'.
